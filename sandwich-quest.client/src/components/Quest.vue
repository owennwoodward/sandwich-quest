    ...
    <div class="container">
      <div class="row justify-content-start">
        <button
          v-if="!questItems == 0"
          :class="{
            'bg-success blur darken-27':
              doneItems.length == questItems.length && questItems.length != 0,
          }"
          class="
            ongoing
            col-10
        ...
        <div
          ...
          class="col-2 text-end bg-primary borders pt-2 selectable corner-right"
          @click.stop="removeQuest"
        >
          <i
            v-if="
              doneItems.length == questItems.length && questItems.length != 0
            "
            class="mdi mdi-check-circle check h4"
          ></i>
          ...
          <div class="collapse collapse-horizontal" :id="`id` + quest.id">
            <div class="card-body collapse-mobile d-flex bg-dark flex-column">
              ...
              <div v-if="questItems.length >= 1" class="progress">
                <div
                  :class="{ green: doneItems.length == questItems.length }"
                  class="progress-bar bg-secondary"
                  role="progressbar"
                  :style="{
                    width:
                      Math.floor((doneItems.length / questItems.length) * 100) +
                      '%',
                  }"
                  aria-valuemin="0"
                  aria-valuemax="100"
              ...
              <h5 v-if="questItems == 0">
                You have no quest items for this quest
              </h5>
...
export default {
  ...
  setup(props) {
    ...
    return {
      getStaticMap,
      questItems: computed(() =>
        AppState.questitems.filter((i) => i.questId == props.quest.id)
      ),

      doneItems: computed(() =>
        AppState.questitems.filter(
          (i) => i.isChecked == true && i.questId == props.quest.id
        )
      ),
